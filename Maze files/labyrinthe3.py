...
class Character:
    ...
    def __init__(self, name, position, bag):
        ...
        # we use the '__init__() method with many parameters, the class
        # constructor
...
class Labyrinthe:
    ...
    def play(self):
        ...
        while(self.character.position != self.end or
              self.character.bag < self.minobject):
            ...
            if c == 't':
                ...
                new_position[0] -= 1
                if self.available_position(new_position):
...
# we check if the character is on a square that contains an object. if
# yes, we collect it
...
# We implement the function which allow us
# to move the character in the direction we want
# Ici on implémente la fonction qui permet de faire bouger le personnage
# en fonction de la direction donnée en argument
...
# On verifie si le personnage est sur une case
# qui contient un objet et on le récupère ou non
# we check if the character is on a square
# that contains an object. if yes, we collect it
        x = self.character.position[0]
        y = self.character.position[1]
