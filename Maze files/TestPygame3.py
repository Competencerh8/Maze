...
# On instancie du texte qu'on va afficher par la suite quand l'utilisateur
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        # Ici on implémente les mouvements du caractère en fonction des
        # instructions de l'utilisateur
        if event.type == pygame.KEYDOWN:

            if((laby.character.position != laby.end
              or laby.character.bag < laby.minobject) and lost == False):
    ...
    if(laby.character.position == laby.end 
      and laby.character.bag == laby.minobject):
